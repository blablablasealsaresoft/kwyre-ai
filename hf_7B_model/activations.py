...
@torch.jit.script
def swiglu_fwd(x, y):
    return x * y / (1.0 + torch.exp(-x))


@torch.jit.script
def swiglu_bwd(x, y, g):
    """
    Computes the backward pass for the SwiGLU activation.
    
    Args:
        x (torch.Tensor): Input tensor x.
        y (torch.Tensor): Input tensor y.
        g (torch.Tensor): Gradient of the output with respect to the function output.
    
    Returns:
        Tuple[torch.Tensor, torch.Tensor]: Gradients with respect to x and y.
    """
    x_sigmoid = 1.0 / (1.0 + torch.exp(-x))
    dx = x_sigmoid * (1.0 + x * (1.0 - x_sigmoid)) * g * y
    dy = x * x_sigmoid * g
    return dx, dy
...
def swiglu_bwd_with_output(x, y, g):
    """
    Computes the backward pass for the SwiGLU activation, including the intermediate output.
    ...
    Returns:
        Tuple[torch.Tensor, torch.Tensor, torch.Tensor]: Gradients with respect to x (dx), 
        y (dy), and the intermediate output z.
    """
    x_sigmoid = 1.0 / (1.0 + torch.exp(-x))
    x_swish = x * x_sigmoid
    dx = x_sigmoid * (1.0 + x * (1.0 - x_sigmoid)) * g * y
